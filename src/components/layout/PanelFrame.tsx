...
interface PanelFrameProps {
  title: string;
  /** Stable id for the panel title (for aria-labelledby on a wrapping region). */
  titleId?: string;
  children: ReactNode;
  className?: string;
}

export function PanelFrame({ title, titleId, children, className = '' }: PanelFrameProps) {
  return (
    <div
    ...
    >
      <div className="flex items-center gap-2 px-4 py-2 border-b border-tactical-border bg-tactical-surface/50">
        ...
        <h2
          id={titleId}
          className="text-sm font-mono text-terminal-green/80 tracking-widest uppercase"
        >
          {title}
        </h2>
      </div>
