...
### UCDP API → Bundled CSV Fix
- UCDP API added auth token requirement in February 2026 — broke historical mode
- Downloaded UCDP GED v25.1 CSV (250MB), filtered to 140,770 events across 7 countries
- Compact JSON format (dictionary-encoded strings, integer-packed coords) → 6.2MB at `public/data/ucdp-ged-filtered.json`
- Created `src/lib/data/ucdpGedData.ts` for client-side decoding (not used by API route)
- Rewrote `src/app/api/historical/route.ts` to read JSON from disk via `fs.readFileSync` with in-memory cache
- Zero external API dependencies — historical mode works fully offline

### Historical Playback Overhaul
- **Cumulative mode** — past years fade to 30% opacity, current year at full brightness, future years hidden. No more jarring flash on year change.
- **Color gradient by year** — blue/cyan (earliest) through yellow/orange to red (latest). Generated by `getYearColor()` in ConflictEventLayer.
- **Fatality-scaled markers** — diamond markers sized 5/7/10/14px based on death count (1-2, 3-9, 10-49, 50+). Major battles (Srebrenica, Fallujah) visually dominate.
- **CSS transitions** — 0.4s ease on opacity for smooth fading between years
- **Running KIA counter** — cumulative fatality count shown in HistoricalTimeline bar, updates via `onFatalityUpdate` callback from ConflictEventLayer
- **Single fetch** — historical events loaded once on mount, filtered client-side. No API re-fetch on year change.

### Historical Source Links (DetailPanel)
- UCDP events now show: Side A / Side B faction names, fatality breakdown (total, side A, side B, civilian)
- WIKIPEDIA link → searches conflict name on Wikipedia
- HISTORICAL COVERAGE link → Google News archive search scoped to conflict + country + year

### Historical Legend (MapLegend)
- MapLegend accepts `historical?: HistoricalConfig` prop from TacticalMap
- When in historical mode, shows:
  - Color gradient bar with start/end year labels
  - Size key: four diamond markers showing fatality scale (1-2, 3-9, 10-49, 50+)
  - Opacity guide: full = current year, faded = past years

### Other Fixes
- Port 3117 restored in `package.json` dev script (was missing, defaulting to 3000)
- Permissions expanded to ~60 rules — covers all common Bash commands, Agent tool, skills
